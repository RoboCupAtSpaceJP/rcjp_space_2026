...
class ScoreDisplay:
    ...
    def _build_ui(self):
        ...
        for label, key, max_pt in SCORE_ITEMS:
            ...
            tk.Label(row, text=f"/ {max_pt}", font=("Helvetica", 12),
                     bg=BG, fg=DIMGRAY, width=6, anchor='e').pack(side='right')
            val = tk.Label(row, text="0", font=("Helvetica", 12, "bold"),
        ...
        tk.Label(total_frame,
                 ...
                 font=("Helvetica", 14), bg=BG, fg=FG).pack()
        self.total_label = tk.Label(total_frame, text=f"0",
                                     font=("Helvetica", 64, "bold"), bg=BG, fg=BLUE)
        self.total_label.pack()
        self.max_label = tk.Label(total_frame, text=f"/ {MAX_TOTAL}",
                                   font=("Helvetica", 16), bg=BG, fg=DIMGRAY)
        self.max_label.pack()
